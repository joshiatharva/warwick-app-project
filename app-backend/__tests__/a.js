...
describe('Auth Endpoints - register', () => {
  ...
  it('should register a user - already exists', async () => {
...
describe("Forgot Password link", () => {
  it('/auth/forgot/ - token present', async () => {
    var user = await User.findOne({ username: "atthujoshi" });
    var token = jwt.sign({ id: user._id }, "This is secret");
    const res = await request.post('/auth/forgot').set('Authorization', 'Bearer ' + token).send({ email: "atthujoshi@gmail.com", url: "abcd.co.uk", path: "/forgot", username: "atthujoshi",});
    expect(res.body.msg).toBe("/");
  });
  it('/auth/forgot - works', async () => {
    const res = await request.post('/auth/forgot').send({ email: "atthujoshi@gmail.com", url: "abcd.co.uk", path: "/forgot", username: "atthujoshi" }).set("Authorization", "Bearer");
    expect(res.status).toBe(400);
...
describe("Reset Password link", () => {
  it('/auth/reset/1234', async () => {
    var user = await User.findOne({ username: "atthujoshi" });
    var token = jwt.sign({ id: user._id }, "This is secret");
    const res = await request.post('/auth/reset/abcd').set("Authorization", "Bearer " + token).send({id: user._id});
    expect(res.body.msg).toBe("/");
  });
...
describe('logout', () => {
  it('logs out', async () => {
    ...
    const res = await request.get("/auth/logout/"+token);
    expect(res.status).toBe(200);
...
afterAll(async () => {
  ...
  await User_forgot_password.deleteMany({});
  mongoose.disconnect();
});